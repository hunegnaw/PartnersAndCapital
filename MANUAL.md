...
- Statement IRR/APR display fix: The first-page IRR and APR metrics now appear whenever **any** of the client's investments has that value set (each is an invested-amount-weighted blend), and a legitimate **0%** now shows instead of being hidden. Two bugs were fixed in `statement-generator.ts`: (1) APR was divided by the IRR weight (a shared `totalWeight` that only counted investments with an IRR set), so if no investment had an IRR, APR was forced to 0 and never displayed even when entered — IRR and APR now track their weights independently (`irrWeight`/`aprWeight`); (2) an unset metric now resolves to `null` (hidden) rather than `0`, and the PDF gate checks `!= null` instead of truthiness, so a real 0% renders. If IRR/APR are blank on the statement, populate `irr` / `adminApr` on the client's positions (admin investment detail page).
