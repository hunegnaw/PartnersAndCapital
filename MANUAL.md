...
- Statement per-investment activity tables: Renamed and rescoped. The current-period table is now **"Current Month Distributions & Credits"** (the statement month's activity). The second table is now **"Previous Distributions & Activity"** and shows **all months of the statement's calendar year** (Jan 1 → statement month) — distributions and contributions/buy-ins — instead of the previous 3-months-prior window. Example: a May 2026 statement lists Jan–May 2026; a Dec 2025 year-end statement lists all of 2025. Driven by the `ytdStart → periodEnd` range on `previousActivity` in `statement-generator.ts`. Long year-end lists paginate per row (note: the column header isn't repeated on continuation pages).
